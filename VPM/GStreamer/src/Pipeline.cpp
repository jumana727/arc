...
CMx_Pipeline::CMx_Pipeline()
{
    ...
    // Update the plugin path based on your setup
    std::string pluginPath = "D:/gstreamer/1.0/msvc_x86_64/lib/gstreamer-1.0"; // Adjusted for Windows
    std::string gioPath = "D:/gstreamer/1.0/msvc_x86_64/lib/gio"; // Adjusted for Windows
    
    g_setenv("GST_PLUGIN_PATH", pluginPath.c_str(), TRUE);
    g_setenv("GIO_MODULE_DIR", gioPath.c_str(), TRUE);
    
    char level[4] = { 0 };
    snprintf(level, sizeof(level), "%d", 3); // GST_DEBUG level

    g_setenv("GST_DEBUG", level, TRUE);
    g_setenv("GST_DEBUG_FILE", "D:/log.txt", TRUE); // Adjusted log file path for Windows
    gst_init(NULL, NULL);
...
void CMx_Pipeline::InitPipeline(int ipipelineID, const char * rtspURL)
{
    ...
    g_object_set(sink, "location", mediaMtxURL.c_str(), nullptr);
...
// Commented out recording functions for now
/*
